...
const options = {
  url: 'https://dev85358.service-now.com',
  username: 'admin',
  password: 'wNlXjxTDB4i7'
};
...
function processRequestResults(error, response, body, callback) {
  ...
  if (response.statusCode === 200 || response.statusCode === 201) {
      if (isHibernating(response)) {
        baderror = `Site is hibernating`
      }
      else {
        results = {statusCode: response.statusCode,body: response.body}
      }
  }
  else {
      baderror = `Something went wrong`
  }
  // var staco = response.statusCode;
  // console.log(staco);
  return callback(results, baderror);
}
